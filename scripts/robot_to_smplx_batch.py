...
def process_single_file_worker(args):
    """处理单个文件（供进程池调用）"""
    index, npz_path, output_path, robot, SMPLX_FOLDER, gender, betas, no_visualize, rate_limit = args
    try:
        # 检查输入文件是否存在
        ...
        # 处理文件
        success = process_single_npz_file(npz_path, output_path, robot, SMPLX_FOLDER, gender, betas, no_visualize, rate_limit)
...
def process_batch_from_csv(csv_file, batch_save_path, robot, SMPLX_FOLDER, gender, betas, no_visualize=False, rate_limit=False, use_multithreading=True, num_threads=None):
    ...
    csv_dir = csv_path.parent if csv_path.parent != pathlib.Path("") else pathlib.Path(".")

    candidate_roots = [
        csv_dir,
        pathlib.Path(__file__).parent.parent.parent / "server3_data",
    ...
    try:
        with open(csv_file, 'r') as csvfile:
            ...
            for row in reader:
                ...
                if relative_path:
                    npz_path = pathlib.Path(relative_path)
                    resolved_path = None
                    if npz_path.is_absolute() and npz_path.exists():
                        resolved_path = npz_path
                    else:
                        for root in candidate_roots:
                            candidate = (root / npz_path).resolve()
                            if candidate.exists():
                                resolved_path = candidate
                                break
                    if resolved_path is None:
                        resolved_path = (csv_dir / npz_path).resolve()
                    file_pairs.append((index, str(resolved_path)))
                row_idx += 1
    except Exception as e:
    ...
    if use_multithreading:
        ...
        # 准备任务参数
        tasks = []
        for index, npz_path in file_pairs:
            output_path = os.path.join(batch_save_path, f"{index}.npz")
            task_args = (index, npz_path, output_path, robot, SMPLX_FOLDER, gender, betas, no_visualize, rate_limit)
            tasks.append(task_args)
        
        # 并行处理所有任务（进程池）
        index_to_path = {index: npz_path for index, npz_path in file_pairs}
        failure_counts = {}
        sample_failures = []
...
# note: whole process of retargeting single npz file  
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--robot_file",
        help="Robot motion NPZ file to load (LaFAN format).",
        type=str,
    )
    ...
    parser.add_argument(
        ...
        default="unitree_g1",
        help="Robot type used to generate the LaFAN NPZ motions.",
    )
    ...
    parser.add_argument(
        "--batch_save_path",
        help="Directory path to save batch processed SMPL-X NPZ motion files.",
        type=str,
        default=None,
    )
    ...
    if args.csv_file and args.batch_save_path:
        ...
        if args.save_path is not None:
            success = process_single_npz_file(args.robot_file, args.save_path, args.robot, SMPLX_FOLDER, args.gender, betas, args.no_visualize, args.rate_limit)
            if not success:
